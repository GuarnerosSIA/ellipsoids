...
grid on
for k = 1:NE
    E(k,:)=BP_ellipsoids(E(k,:),100,1);%500
    plotellipsoid(E(k,:))
    Classifier.datos_clases{k} = E{k,4};
    Classifier.Ellipsoids{1,k}=E{k,1};
    Classifier.Ellipsoids{2,k}=E{k,2};
    Classifier.Ellipsoids{3,k}=E{k,3};
...
SSE = Classifier.clusterValidation(data);
[E, Classifier] = eliminateClusterSSE(E,SSE,0.45,Classifier); %0.45
end
